...
public class ScheduledService implements TaskMXBean {
    ...
    private void init() throws Exception {
        ...
        if (!mbs.isRegistered(name)) {
            //mbs.registerMBean(this, name); я не знаю, почему оно ломается, ему кажется, что уже такой MBean есть
        } else {//причем проверку он проходит (что MBean нет), НО сразу же находит его и гг
